...
老实说我们把每一个 seata-python 用户都视为非常善良的贡献者。在体验了 seata-python 之后，您可能会对项目有一些反馈。然后随时通过 [NEW ISSUE](https://github.com/seata/seata-python/issues/new/choose)打开一个问题。
...
1. **FORK** seata-python 到您的存储库。要完成这项工作，您只需单击 [seata/seata-python](https://github.com/seata/seata-python) 主页右侧的 Fork 按钮。然后你将在 中得到你的存储库`https://github.com/<your-username>/seata-python`，其中 your-username 是你的 GitHub 用户名。

2. **克隆** 您自己的存储库以在本地开发. 用于 `git clone git@github.com:<your-username>/seata-python.git` 将存储库克隆到本地计算机。 然后您可以创建新分支来完成您希望进行的更改。

3. **设置远程** 将上游设置为 `git@github.com:seata/seata-python.git` 使用以下两个命令：
...
现在我们假设通过拉取请求的每个贡献都是针对 seata-python 中的 [开发分支](https://github.com/seata/seata-python/tree/master) 。在贡献之前，请注意分支定义会很有帮助。
...
另外，在代码变更部分，我们建议所有贡献者阅读 seata-python 的 [代码风格](#代码风格)。
...
PR 是更改 seata-python 项目文件的唯一方法。为了帮助审查人更好地理解你的目的，PR 描述不能太详细。我们鼓励贡献者遵循 [PR 模板](./.github/PULL_REQUEST_TEMPLATE.md) 来完成拉取请求。
...
Seata-python 代码风格参考[Flake8](https://flake8.pycqa.org/en/latest/) 。
